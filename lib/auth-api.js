...
export const verifyOtp = async (otp) => {
  try {
    ...
    const response = await api.post("/auth/verify-otp/", { otp }, {
      headers: { Authorization: `Bearer ${signupToken}` }, // Adjust based on backend
    });
...
export const forgetPassword = async (val) => {
  try {
    const response = await api.post("/auth/forget-password/", { val });
...
export const validateForgetPasswordOtp = async (otp, token) => {
  try {
    const response = await api.post("/auth/validate-forget-password-otp/", {
      otp,
      token,
    });
    console.log("Validate Forget Password OTP Response:", response);
    return response;
  } catch (error) {
    console.error("Validate Forget Password OTP Error:", error);
    throw error;
  }
};
...
export const resendForgetPasswordOtp = async (token) => {
  try {
    const response = await api.post("/auth/resend-forget-password-otp/", {
      token,
    });
    console.log("Resend Forget Password OTP Response:", response);
    return response;
  } catch (error) {
    console.error("Resend Forget Password OTP Error:", error);
    throw error;
  }
};
...
const saveAuthTokens = (data, isSignup = false) => {
  ...
  const now = new Date();
  const accessExpiryDate = new Date(now.getTime() + data.access_expiry * 1000);
  ...
  if (isSignup) {
    // Set signup_token instead of access_token for signup flow
