...
/**
 * Retrieves details of a specific loan (secured or unsecured)
 * @param {number} id - Loan ID
 * @param {string} type - Loan type ("secured" or "unsecured")
 * @returns {Promise} Response object
 * Request Type: GET
 * Endpoint: /api/loan/{type}-loan/{id}/
 */
export const getLoanDetail = async (id, type) => {
  try {
    const endpoint = type === "secured" ? `/loan/secured-loan/${id}/` : `/loan/unsecured-loan/${id}/`;
    const response = await api.get(endpoint);
    console.log(`Get ${type} Loan Details Response:`, response);
    return response;
  } catch (error) {
    console.error(`Get ${type} Loan Details Error:`, error);
    throw error;
  }
};