...
import { getPropertyInsuranceDetail, deletePropertyInsurance } from "@/lib/property-insurance-api";
import { deleteNominee } from "@/lib/nominee-api";
import { toast } from "sonner";

const PropertyCard = ({ propertyInsurance, onEdit, onDelete }) => {
  ...
  const [detailedInsurance, setDetailedInsurance] = useState(propertyInsurance);

  const fetchPropertyDetails = async () => {
    try {
      setLoadingDetails(true);
      const response = await getPropertyInsuranceDetail(propertyInsurance.id);
      ...
      console.error("Error fetching property insurance details:", error);
      toast.error("Failed to load property insurance details");
    } finally {
  ...
  const handleConfirmDelete = async () => {
    try {
      const response = await deletePropertyInsurance(propertyInsurance.id);
      if (response.status) {
        onDelete(propertyInsurance.id);
  ...
  const handleConfirmDeleteNominee = async () => {
    ...
    try {
      const response = await deleteNominee("property", selectedNominee.id);
      if (response.status) {
    ...
    <>
      <Card className="p-0 transition-all hover:shadow-lg hover:-translate-y-1">
        <CardHeader className="p-6 pb-0">
          <CardTitle>{detailedInsurance.policy_type || "Loading..."}</CardTitle>
        </CardHeader>
        <CardContent>
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
            ...
            <div className="bg-popover p-2 rounded-lg">
              ...
              <p className="font-semibold mt-2 truncate">{detailedInsurance.property_address || "N/A"}</p>
            </div>
            <div className="bg-popover p-2 rounded-lg">
            ...
            <div className="bg-popover p-2 rounded-lg">
              ...
              <p className="font-semibold mt-2 truncate">
                {detailedInsurance.policy_start_date
                  ? new Date(detailedInsurance.policy_start_date).toLocaleDateString()
                  : "N/A"}
              </p>
            </div>
            <div className="bg-popover p-2 rounded-lg">
              ...
              <p className="font-semibold mt-2 truncate">
                {detailedInsurance.policy_expiry_date
                  ? new Date(detailedInsurance.policy_expiry_date).toLocaleDateString()
                  : "N/A"}
              </p>
            </div>
          </div>
            ...
            {documentUrl && (
              <>
                {isImage ? (
                  <img
                    src={documentUrl}
                    alt="Property Insurance Document"
      ...
      <Dialog open={deleteDialogOpen} onOpenChange={setDeleteDialogOpen}>
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>
              Are you sure you want to delete the property insurance "{detailedInsurance.policy_type}"? This action cannot be undone.
            </DialogDescription>
          </DialogHeader>
          <DialogFooter>
