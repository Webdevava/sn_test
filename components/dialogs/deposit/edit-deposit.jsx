...
import { updateDeposit } from "@/lib/deposit-api";
import { toast } from "sonner";

const EditDepositDialog = ({ open, onOpenChange, deposit, onSuccess }) => {
  ...
  useEffect(() => {
    if (deposit) {
      setFormData({
        ...
        installment: deposit.installment?.toString() || "",
        installment_type: deposit.installment_type || "Monthly",
        interest_rate: deposit.interest_rate?.toString() || "",
        tenure: deposit.tenure?.toString() || "",
        maturity_date: deposit.maturity_date || "",
        maturity_amount: deposit.maturity_amount?.toString() || "",
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const response = await updateDeposit(deposit.deposit_type, deposit.id, payload);
      if (response.status) {
