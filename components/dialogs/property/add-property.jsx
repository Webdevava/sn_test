...
const AddPropertyDialog = ({ open, onOpenChange, onSuccess }) => {
  ...
  const validateInsurance = () => {
    ...
    if (!insuranceData.property_address || insuranceData.property_address.trim().length < 1) newErrors.property_address = "Property address is required";
  ...
  const handleInsuranceSubmit = async (e) => {
    ...
    setLoading(true);
    try {
      const response = await createPropertyInsurance({
        ...insuranceData,
  ...
  const handleAddNominee = async () => {
    ...
    setLoading(true);
    try {
      const response = await createNominee("property", {
  ...
  const handleFileSubmit = async (e) => {
    ...
    try {
      if (file) {
        ...
        toast.success("Property insurance saved without document");
      }
      onSuccess();
      handleClose();
    } catch (error) {
    ...
    <Dialog open={open} onOpenChange={handleClose}>
      <DialogContent className="sm:max-w-[600px] p-0 h-[85vh] flex flex-col">
        <DialogHeader className="p-4 border-b">
          <DialogTitle>
            {step === 1 ? "Add Property Insurance" : step === 2 ? "Add Nominees" : "Upload Document"}
          </DialogTitle>
        </DialogHeader>
              ...
              <div className="grid grid-cols-2 gap-4">
                <div className="grid gap-2">
                  ...
                  <Select
                    ...
                    onValueChange={(value) =>
                      handleInsuranceChange({ target: { id: "policy_type", value } })
                    }
                    required
                  >
                    <SelectTrigger>
                ...
                <div className="grid gap-2">
                  ...
                  <Input
                    id="property_address"
                    value={insuranceData.property_address}
                    onChange={handleInsuranceChange}
                    required
