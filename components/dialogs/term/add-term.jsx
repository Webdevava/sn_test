...
import { createTermInsurance, uploadTermInsuranceDocument } from "@/lib/term-insurance-api";
...
const AddTermDialog = ({ open, onOpenChange, onSuccess }) => {
  ...
  const validateInsurance = () => {
    ...
    if (!insuranceData.sum_assured || isNaN(insuranceData.sum_assured) || parseFloat(insuranceData.sum_assured) <= 0)
  ...
  const handleInsuranceSubmit = async (e) => {
    ...
    setLoading(true);
    try {
      const response = await createTermInsurance({
        ...insuranceData,
        sum_assured: parseFloat(insuranceData.sum_assured),
        premium_amount: parseFloat(insuranceData.premium_amount),
        policy_term: parseInt(insuranceData.policy_term),
        coverage_detail: insuranceData.coverage_detail.length ? insuranceData.coverage_detail.split(",") : [],
      });
      if (response.status) {
  ...
  const handleAddNominee = async () => {
    ...
    setLoading(true);
    try {
      const response = await createNominee("term_insurance", {
  ...
  const handleFileSubmit = async (e) => {
    ...
    try {
      if (file) {
        const response = await uploadTermInsuranceDocument(insuranceId, file);
        if (response.status) {
    ...
    <Dialog open={open} onOpenChange={handleClose}>
      <DialogContent className="sm:max-w-[600px] p-0 h-[85vh] flex flex-col">
        <DialogHeader className="p-4 border-b">
          <DialogTitle>
            {step === 1 ? "Add Term Insurance" : step === 2 ? "Add Nominees" : "Upload Document"}
          </DialogTitle>
        </DialogHeader>
              ...
              <div className="grid grid-cols-2 gap-4">
                ...
                <div className="grid gap-2">
                  ...
                  <Input
                    id="sum_assured"
                    type="text"
                    value={insuranceData.sum_assured}
                    onChange={handleInsuranceChange}
                    required
                ...
                <div className="grid gap-2 col-span-2">
                  ...
                  <Input
                    id="coverage_detail"
                    value={insuranceData.coverage_detail}
                    onChange={handleInsuranceChange}
                ...
                <div className="grid gap-2">
                  ...
                  <Select
                    ...
                    onValueChange={(value) =>
                      handleInsuranceChange({ target: { id: "installment_type", value } })
                    }
                    required
                  >
                    <SelectTrigger>
