// @/components/EditMutualFundDialog.jsx
"use client";
...
import { updateAlreadyHeldMutualFund, updateRecurringMutualFund } from "@/lib/mutual-fund-api";
import { toast } from "sonner";

const EditMutualFundDialog = ({ open, onOpenChange, mutualFund, onSuccess }) => {
  ...
  useEffect(() => {
    if (mutualFund) {
      setFormData({
        fund_name: mutualFund.fund_name || "",
        invested_amount: mutualFund.invested_amount?.toString() || "",
        units: mutualFund.units?.toString() || "",
        fund_type: mutualFund.fund_type || "",
        folio_number: mutualFund.folio_number || "",
        frequency: mutualFund.frequency || "Other",
        sip_amount: mutualFund.sip_amount?.toString() || "",
        sip_frequency: mutualFund.sip_frequency || "",
        sip_date: mutualFund.sip_date?.toString() || "",
      });
    }
  }, [mutualFund]);
  ...
  const validateForm = () => {
    ...
    if (mutualFund?.categroy === "Already Invested") {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (mutualFund.categroy === "Already Invested") {
        ...
        response = await updateAlreadyHeldMutualFund(mutualFund.id, payload);
      } else {
        const payload = {
          fund_name: formData.fund_name,
        ...
        response = await updateRecurringMutualFund(mutualFund.id, payload);
      }
      if (response.status) {
        ...
        throw new Error(response.message || "Failed to update mutual fund");
      }
    } catch (error) {
      toast.error(error.message || "Failed to update mutual fund");
    } finally {
      setLoading(false);
    }
  };
        ...
        <form onSubmit={handleSubmit} className="flex flex-col h-full">
          <div className="flex-1 overflow-y-auto">
            <div className="grid gap-4 p-4">
              ...
              {mutualFund?.categroy === "Already Invested" ? (
                <>
...
export default EditMutualFundDialog;